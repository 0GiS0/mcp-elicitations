...
server.registerTool("search-videos", {
    title: "Search Videos",
    ...
    const results = [
        {
            ...
            url: `https://example.com/video/${query.replace(/\s+/g, "-")}-${language.toLowerCase()}`,
            description: `A video discussing ${query} in ${language}.`,
        },
        {
            title: `Another video about ${query} in ${language}`,
            url: `https://example.com/video/${query.replace(/\s+/g, "-")}-${language.toLowerCase()}-2`,
